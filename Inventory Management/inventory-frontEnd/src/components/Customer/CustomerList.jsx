...
const CustomerList = () => {
    ...
    useEffect(() => {
        (async () => {
            await CustomerListRequest(1, perPageKey, searchKey);
        })();
    ...
    const handlePageClick = async (event) => {
        ...
        await CustomerListRequest(event.selected + 1, perPageKey, searchKey);
    };
    ...
    const PageKeyOnChange = async (e) => {
        ...
        setPerPageKey(value);
        setCurrentPage(1);
        await CustomerListRequest(1, value, searchKey);
    ...
    const searchOnChange = async (e) => {
        ...
        setSearchKey(value);
        await CustomerListRequest(1, perPageKey, value);
        console.log(Total);
    };
                                                    ...
                                                    <tbody>
                                                    {
                                                        DataList.map((item, i) =>
                                                            ...
                                                            <tr key={item["_id"]}>
                                                                ...
                                                                <td>
                                                                    <p className="text-xs text-start">{item["customer_name"]}</p>
                                                                </td>
                                                                <td>
                                                                    <p className="text-xs text-start">{item["customer_phone"]}</p>
                                                                </td>
                                                                <td>
                                                                    <p className="text-xs text-start">{item["customer_address"]}</p>
                                                                </td>
                                                                <td>
                                                                    <p className="text-xs text-start">{item["customer_email"]}</p>
                                                                </td>
